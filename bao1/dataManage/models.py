...
class boke(models.Model):
    ...
    user_id = models.OneToOneField(to='user1',to_field='id',on_delete=None,verbose_name='博客主人')
    def __srt__(self):
...
class fufen(models.Model):
    star_Id = models.ForeignKey(user1,verbose_name='明星',on_delete=None,parent_link=True,related_name='user_star',)
    fans_id = models.ForeignKey(user1,verbose_name='粉丝',on_delete=None,parent_link=True,related_name='user_fans',)
    class Meta:
        verbose_name = "粉丝表"
...
class ensure(models.Model):
    ...
    T_user = models.ForeignKey(user1,verbose_name='提问人',on_delete=None,parent_link=True, related_name='T_user')
    D_user = models.ForeignKey(user1,null=True,verbose_name='回答人',on_delete=None,parent_link=True,related_name='D_user')
    state_s = {
        (0,'未处理'),
        (1, '处理中'),
        (2, '已处理')
...
class article(models.Model):
    ...
    author = models.ForeignKey(user1,on_delete=None,parent_link=True,verbose_name='作者')
    label_id = models.ManyToManyField(to="label",verbose_name='文章标签',related_name='article_label',related_query_name='article_label', )
    zyfenlei_s = {
        (0, 'python'),
        (1, 'java'),
        (2, 'php'),
    ...
    zyfenlei = models.IntegerField(choices=zyfenlei_s, default=0, verbose_name='主页分类')
    article_nr_id = models.ForeignKey(to='article_nr',on_delete=None)
    class Meta:
        verbose_name = "文章信息"
...
class article_nr(models.Model):
    content = models.CharField(max_length=255,verbose_name='内容')
    # article_xx_id = models.OneToOneField(to='article' ,on_delete=None,verbose_name='文章信息')
    class Meta:
        verbose_name = "文章内容"
...
class article_zan(models.Model):
    ...
    user_id = models.ForeignKey(user1,on_delete=None,parent_link=True,verbose_name='用户')
